    ...
    <div class="card">
        <div class="card-body">
            <div class="card-title d-flex justify-content-between align-items-center">
                <h6>ویرایش نمونه کار</h6>
            </div>
            <form id="product-form" action="{{ route('skill.update', $skill->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-row">
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-3">
                        <label for="title">عنوان<span class="text-danger">*</span></label>
                        <input type="text" name="title" class="form-control" id="title" value="{{ old('title', $skill->title) }}">
                        @error('title')
                ...
                <div class="col-xl-12 col-lg-12 col-md-12 mb-3">
                    ...
                    <textarea name="description" class="form-control textarea" id="description">{{ old('description', $skill->description) }}</textarea>
                    @error('description')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped text-center" id="properties_table">
                        ...
                        <tbody>
                        @foreach($skill->properties as $key => $property)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>
                                    <input type="text" name="properties[]" class="form-control" value="{{ $property }}" required>
                                </td>
                                <td>
                ...
                <button class="btn btn-primary" type="submit">بروزرسانی فرم</button>
            ...
            form.on('submit', function (event) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        window.location.href = "{{ route('skill.index') }}";
                    },
                    error: function (xhr) {
                        modal.hide();
                        if (xhr.status === 422) {
