...
class PanelController extends Controller
{
    public function index(Request $request)
    {
        $from_date = $request->from_date
            ...
            : optional(Invoice::orderBy('created_at')->first())->created_at ?? Carbon::now()->toDateTimeString();

        $to_date = $request->to_date
            ? Verta::parse($request->to_date)->endDay()->toCarbon()->toDateTimeString()
            : optional(Invoice::orderBy('created_at', 'desc')->first())->created_at ?? Carbon::now()->toDateTimeString();
        ...
        $from_date2 = $request->from_date
            ...
            : optional(UserVisit::orderBy('created_at')->first())->created_at ?? Carbon::now()->startOfDay();

        $to_date2 = $request->to_date
            ? Verta::parse($request->to_date)->toCarbon()->endOfDay()
            : optional(UserVisit::orderBy('created_at', 'desc')->first())->created_at ?? Carbon::now()->endOfDay();
...
// تبدیل تاریخ‌ها به شمسی
        ...
        $from_date3 = $request->from_date
            ? Verta::parse($request->from_date)->toCarbon()->startOfDay()
            : optional(Sms::orderBy('created_at')->first())->created_at ?? Carbon::now()->startOfDay();

        $to_date3 = $request->to_date
            ? Verta::parse($request->to_date)->toCarbon()->endOfDay()
            : optional(Sms::orderBy('created_at', 'desc')->first())->created_at ?? Carbon::now()->endOfDay();
